...
public class FileAudioLauncher  implements MediaLauncher {
	   ...
	   MultipleFilesInputStream audio_input_stream=null;
		   ...
		   log=logger;
		      ...
		      {  socket=new DatagramSocket(local_port);
		         ...
		         {  printLog("new audio sender to "+remote_addr+":"+remote_port,LogLevel.MEDIUM);
		            ...
		            //sender=new RtpStreamSender(audio_input_stream,false,payload_type,frame_rate,frame_size,socket,remote_addr,remote_port);
		            audio_input_stream = new MultipleFilesInputStream();
		            sender=new RtpStreamSender(audio_input_stream,true,payload_type,frame_rate,frame_size,socket,remote_addr,remote_port);
		            sender.setSyncAdj(2);
		         }
		         // receiver
		         if (dir<=0)
	   ...
	   public void addFileToPlay(String file_name)
	   {
		   if (null != audio_input_stream) {
			   audio_input_stream.addFileToPlay(file_name);
		   }
	   }
	   ...
	   {  if (log!=null) log.println("FileAudioLauncher: "+str,level+SipStack.LOG_LEVEL_UA);
	      if (level<=LogLevel.HIGH) System.out.println("FileAudioLauncher: "+str);
	   }
